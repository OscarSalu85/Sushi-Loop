...
public class cintaManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (time > spawnTime) {
            Instantiate(prefab);
            prefab.transform.position = new Vector3(prefab.transform.position.x, -8f, prefab.transform.position.z);
            ...
            if(prefab.GetComponent<sushiManager>().sushi == null) Destroy(prefab);
